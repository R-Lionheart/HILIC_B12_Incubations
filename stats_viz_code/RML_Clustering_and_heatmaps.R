...
## read data ------------------
all.dat.no.IS <- read.csv("data_processed/IsoLagran1_0.2_notnormd.csv", stringsAsFactors = FALSE) %>%
  select(-1)

wide.all.data <- all.dat.no.IS %>%
  spread(Mass.Feature, Adjusted.Area) %>%
  column_to_rownames("Replicate.Name")
...
metabs.w.few <- wide.all.data %>%
  gather(Mass.Feature, Adjusted.Area) %>%
