...
BMISd.data.filtered <- BMISd.data %>%
  ...
  filter(Metabolite.Name %in% Full.data.RF.dimensions$Metabolite.Name) %>%
...
IS.mid_frame2 <- lapply(IS.mid_frame,
                        function(x) mutate(x,
                                           umol.in.vial_IS = (Area.with.QC[Std.Type == "Standard"] / 
                                                                Area.with.QC[Std.Type == "Internal_std"]) * 
                                             (Concentration_nM[Std.Type == "Standard"]/1000)))
...
All.Info <- Quantitative.data %>%
  select(Metabolite.Name, runDate:replicate, Adjusted.Area, Area.with.QC, everything()) %>%
  unite(Sample.Name, c("runDate", "type", "SampID", "replicate"), remove = TRUE) %>%
...
All.Info.Molecules <- All.Info.Quantitative  %>%
  ...
  select(Metabolite.Name, Replicate.Name, everything())
